...
const Dashboard = ({ user, onLogout }) => {
  ...
  // --- PERSISTENCE & EMAIL MEMORY LOGIC ---
  useEffect(() => {
  ...
  const handleGenerateQR = (e) => {
    ...
    localStorage.setItem(`nacos_session_${user.staff_id}`, JSON.stringify({
      qrPayload, level: selectedLevel, course: selectedCourse, hall: selectedHall
    }));
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch final count");
    }

    localStorage.removeItem(`nacos_session_${user.staff_id}`);
    setActiveQR(null);
    setSessionSummary({ course: selectedCourse, hall: selectedHall, count: finalCount });
  };
  ...
  const handleEmailReport = async (e, courseOverride = null) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        alert("✅ Success! Excel Report sent to " + lecturerEmail);
      } else {
      ...
      scanner.render(
        (decodedText) => {
          ...
          try {
            ...
            setScanMessage({ type: 'success', text: `✅ Code accepted for ${qrData.course}. Fetching GPS location...` });
            ...
            if ("geolocation" in navigator) {
              navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                try {
                  ...
                  setScanMessage({ type: 'error', text: "❌ Could not connect to server." });
                }
              }, (error) => {
                setScanMessage({ type: 'error', text: "❌ GPS permission denied. Cannot mark attendance." });
              });
            } else {
              setScanMessage({ type: 'error', text: "❌ GPS not supported on this device." });
            }
          } catch (e) {
  ...
  }, [isScanning, user.role, user.matric_no, apiUrl]);
        ...
        <div className="flex justify-between items-center border-b pb-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 font-medium mt-1">
              {user.role === 'student' ? `Matric No: ${user.matric_no}` : `Staff ID: ${user.staff_id}`}
            </p>
          </div>
        ...
        <div className="py-4">
            ...
            <div className="max-w-xl mx-auto">
                ...
                <>
                  {!activeQR && !sessionSummary && (
                    <form onSubmit={handleGenerateQR} className="space-y-4 animate-fade-in">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Select 2nd Semester Course</label>
                        <select required value={selectedCourse} onChange={(e) => setSelectedCourse(e.target.value)} disabled={!selectedLevel} className={`w-full px-4 py-2 border border-gray-300 rounded-lg outline-none bg-white transition ${!selectedLevel ? 'opacity-50 cursor-not-allowed bg-gray-100' : 'focus:ring-2 focus:ring-nacosGreen'}`}>
                          <option value="" disabled>{!selectedLevel ? '-- Pick a level first --' : '-- Choose a course --'}</option>
                          {selectedLevel && coursesByLevel[selectedLevel].map(course => <option key={course} value={course}>{course}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Select Hall/Location</label>
                  ...
                  {activeQR && (
                    <div className="flex flex-col items-center animate-fade-in text-center">
                      ...
                      <p className="text-gray-500 mb-6">{selectedHall}</p>
                  ...
                  {sessionSummary && (
                    <div className="bg-white p-8 rounded-xl shadow-lg border-2 border-nacosGreen text-center animate-fade-in">
                      <h3 className="text-2xl font-bold text-gray-800 mb-1">Session Ended</h3>
                      <p className="text-gray-500 font-medium mb-6">{sessionSummary.course} • {sessionSummary.hall}</p>
                      
                      <div className="bg-gray-50 border border-gray-200 rounded-xl py-6 mb-6">
                        <p className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-1">Total Sign-Ins</p>
                      ...
                      <form onSubmit={(e) => handleEmailReport(e)} className="mb-4">
                        <label className="block text-sm font-medium text-gray-700 text-left mb-1">Lecturer Email Address</label>
                        <input 
                          type="email" 
                          required
                          list="recent-emails"
                        ...
                        <button type="submit" disabled={isEmailing} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg shadow-md transition duration-200 flex justify-center items-center gap-2">
                          {isEmailing ? "Preparing Email..." : "Email Excel Report"}
                        </button>
                      </form>
              ...
              {activeTab === 'history' && (
                <div className="animate-fade-in">
                  {isLoadingHistory ? (
                    <p className="text-center text-gray-500 py-10">Loading past sessions...</p>
                  ) : historyData.length === 0 ? (
                    <div className="text-center py-10 bg-gray-50 rounded-xl border border-gray-200">
                      <p className="text-gray-500 font-medium">No past classes found.</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <div className="mt-8 pt-6 border-t border-gray-200">
                         <h4 className="text-sm font-bold text-gray-700 mb-2">Need a past report?</h4>
                         <div className="flex space-x-2">
                           <input 
                            ...
                            <button 
                              onClick={() => {
                                const course = prompt("Enter the exact course code to request (e.g., CSC221):");
                                if (course && lecturerEmail) handleEmailReport(null, course);
                              }}
          ...
          {user.role === 'student' && (
            <div className="max-w-md mx-auto text-center">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Student Scanner</h2>
              {scanMessage && (
              ...
              {!isScanning ? (
                <button onClick={() => { setIsScanning(true); setScanMessage(null); }} className="w-full bg-nacosGreen hover:bg-green-800 text-white font-semibold py-4 rounded-xl shadow-lg transition duration-200 flex items-center justify-center space-x-2">
                  ...
                  <span>Tap to Scan Attendance</span>
                </button>
              ) : (
